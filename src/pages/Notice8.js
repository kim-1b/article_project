...
function Notice8() {
  ...
  const [notice8, setNotice8] = React.useState([]);
  ...
  const 게시글정보가져 = async () => {
    await axios({
      url: "http://localhost:4000/notice8",
      method: "GET",
    }).then((response) => {
      setNotice8(response.data);
    });
  };
  ...
  const 작성 = async () => {
    await axios({
      ...
      if (response.data.code === "success") {
        window.location.replace("/notice8");
      }
    });
  };
      ...
      {modalOpen && (
        <div>
          ...
          <div className="modal">
            ...
            <div className="pre-box-1">아르헨티나</div>
            <button
              onClick={() => {
                setNum(num + 1);
              }}
            ...
            <span>
              ...
              <div className="pre-box-2">호주</div>
