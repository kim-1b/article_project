...
function Notice7() {
  ...
  const [notice7, setNotice7] = React.useState([]);
  ...
  const 게시글정보가져 = async () => {
    await axios({
      url: "http://localhost:4000/notice7",
      method: "GET",
    }).then((response) => {
      setNotice7(response.data);
    });
  };
  ...
  const 작성 = async () => {
    await axios({
      ...
      if (response.data.code === "success") {
        window.location.replace("/notice7");
      }
    });
  };
      ...
      {modalOpen && (
        <div>
          ...
          <div className="modal">
            ...
            <div className="pre-box-1">일본</div>
            <button
              onClick={() => {
                setNum(num + 1);
              }}
            ...
            <span>
              ...
              <div className="pre-box-2">크로아티아</div>
