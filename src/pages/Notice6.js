...
function Notice6() {
  ...
  const [notice6, setNotice6] = React.useState([]);
  ...
  const 게시글정보가져 = async () => {
    await axios({
      url: "http://localhost:4000/notice6",
      method: "GET",
    }).then((response) => {
      setNotice6(response.data);
    });
  };
  ...
  const 작성 = async () => {
    await axios({
      ...
      if (response.data.code === "success") {
        window.location.replace("/notice6");
      }
    });
  };
      ...
      {modalOpen && (
        <div>
          ...
          <div className="modal">
            ...
            <div className="pre-box-1">프랑스</div>
            <button
              onClick={() => {
                setNum(num + 1);
              }}
            ...
            <span>
              ...
              <div className="pre-box-2">폴란드</div>
