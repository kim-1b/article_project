...
function Notice5() {
  ...
  const [notice5, setNotice5] = React.useState([]);
  ...
  const 게시글정보가져 = async () => {
    await axios({
      url: "http://localhost:4000/notice5",
      method: "GET",
    }).then((response) => {
  ...
  const 작성 = async () => {
    await axios({
      ...
      if (response.data.code === "success") {
        window.location.replace("/notice5");
      }
    });
  };
      ...
      {modalOpen && (
        <div>
          ...
          <div className="modal">
            ...
            <div className="pre-box-1">네덜란드</div>
            <button
              onClick={() => {
                setNum(num + 1);
              }}
            ...
            <span>
              ...
              <div className="pre-box-2">미국</div>
