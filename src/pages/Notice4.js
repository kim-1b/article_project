...
function Notice4() {
  ...
  const [notice4, setNotice4] = React.useState([]);
  ...
  const 게시글정보가져 = async () => {
    await axios({
      url: "http://18.183.82.9:4000/notice4",
      method: "GET",
    }).then((response) => {
      setNotice4(response.data);
    });
  };
  ...
  const 작성 = async () => {
    await axios({
      ...
      if (response.data.code === "success") {
        window.location.replace("/notice4");
      }
    });
  };
      ...
      {modalOpen && (
        <div>
          ...
          <div className="modal">
            ...
            <div className="pre-box-1">포르투갈</div>
            <button
              onClick={() => {
                setNum(num + 1);
              }}
            ...
            <span>
              ...
              <div className="pre-box-2">스위스</div>
            ...
            <button className="match-box-btn2">
              <span className="match-team2">
                <span className="match-team-img2">
                  <img
                    className="qw"
                    src="https://sports-phinf.pstatic.net/team/qatar2022/default/4699.png"
                  />
                </span>
              </span>
            </button>
