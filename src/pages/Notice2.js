...
function Notice2() {
  ...
  const 게시글정보가져 = async () => {
    await axios({
      url: "http://18.183.82.9:4000/notice2",
      method: "GET",
    }).then((response) => {
      setNotice2(response.data);
    });
  };
  ...
  const 작성 = async () => {
    await axios({
      ...
      if (response.data.code === "success") {
        window.location.replace("/notice2");
      }
    });
  };
      ...
      {modalOpen && (
        <div>
          ...
          <div className="modal">
            ...
            <div className="pre-box-1">잉글랜드</div>
            <button
              onClick={() => {
                setNum(num + 1);
              }}
            ...
            <span>
              ...
              <div className="pre-box-2">세네갈</div>
